...
@Service
public class OpenAIEndgameService {
    ...
    public Map<String, Object> startNewEndgameBattle(int level, String mode, Board initialBoard, Side startingSide) {
        ...
        response.put("message", "Endgame AI battle started - OpenAI " + (mode.equals("cot") ? "CoT" : "ZeroShot") + "(Red) vs Pikafish(Black)");
        response.put("gameId", state.gameId);
        response.put("level", level);
        response.put("mode", mode);
        response.put("board", serializeBoard(state.board));

        System.out.println("[OpenAI Endgame AI Battle] Starting new endgame AI battle: Level " + level +
                " - OpenAI " + (mode.equals("cot") ? "CoT" : "ZeroShot") + "(Red) vs Pikafish(Black)");
        return response;
    }
    ...
    public Map<String, Object> playNextRound(int level, String mode) {
        ...
        System.out.println("[OpenAI Endgame AI Battle] Level " + level + " Round " + state.currentRound + " starting");

        Map<String, Object> openAIResult = playOpenAIMove(state, level, mode);
        if (!(Boolean) openAIResult.get("success")) {
            state.gameOver = true;
            state.gameResult = "Pikafish wins - OpenAI cannot continue";
            state.winner = "BLACK";
            recordEndgameFoul(state, state.currentRound, "Illegal move", "OpenAI");
            recordEndgameGameResult(state, "OpenAI cannot continue");
            saveEndgameErrorsToCSV(state);
            saveEndgameAccuracyToCSV(state);
            ...
            response.put("foulPlayer", "OpenAI");
            response.put("foulReason", openAIResult.get("errorMessage"));
            response.put("boardState", serializeBoard(state.board));
            return response;
        }

        String openAIMove = (String) openAIResult.get("move");
        System.out.println("[OpenAI Endgame AI Battle] OpenAI" + (mode.equals("cot") ? " CoT" : "") + " move: " + openAIMove);
        ...
        if (noCrossingPieces(state.board)) {
            ...
            response.put("winner", state.winner);
            response.put("gameResult", state.gameResult);
            response.put("openAIMove", openAIMove);
            response.put("round", state.currentRound);
            response.put("boardState", serializeBoard(state.board));
            return response;
        }
        ...
        if (XqEndgameJudge.isCheckmated(state.board, Side.BLACK)) {
            state.gameOver = true;
            state.gameResult = "OpenAI wins - checkmate";
            state.winner = "RED";
            recordEndgameGameResult(state, "openai win");
            saveEndgameErrorsToCSV(state);
            saveEndgameAccuracyToCSV(state);
            response.put("success", true);
            response.put("gameOver", true);
        ...
        List<String> openAILegalMoves = getAllLegalMoves(state.board, Side.RED);
        if (openAILegalMoves.isEmpty()) {
            state.gameOver = true;
            state.gameResult = "Pikafish wins - OpenAI has no legal moves";
            state.winner = "BLACK";
        ...
        if (!(Boolean) pikafishResult.get("success")) {
            state.gameOver = true;
            state.gameResult = "OpenAI wins - Pikafish cannot continue";
            state.winner = "RED";
        ...
        System.out.println("[OpenAI Endgame AI Battle] Pikafish move: " + pikafishMove);
    ...
    private void saveEndgameAccuracyToCSV(EndgameAIBattleState state) {
        ...
        try {
            ...
            String fullMode = "openai-" + state.mode;
    ...
    // Executes OpenAI's move for the current round.
    private Map<String, Object> playOpenAIMove(EndgameAIBattleState state, int level, String mode) {
        Map<String, Object> result = new HashMap<>();

        if (openAIClient == null) {
            System.err.println("OpenAI client not initialized");
            result.put("success", false);
            result.put("errorMessage", "OpenAI client not initialized");
            return result;
        }

        if (XqEndgameJudge.isCheckmated(state.board, Side.RED)) {
            System.out.println("OpenAI is checkmated, cannot move");
            result.put("success", false);
        ...
        if (legalMoves.isEmpty()) {
            System.out.println("OpenAI is stalemated, cannot move");
            result.put("success", false);
            result.put("errorMessage", "Red is stalemated, no legal moves");
            return result;
        }

        System.out.println("OpenAI analyzing... legal moves: " + legalMoves.size());
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                if ("zeroshot".equals(mode)) {
                    try {
                        rawResponse = openAIClient.chat(sessionId, boardDescription);
                        if (rawResponse == null || rawResponse.trim().isEmpty()) {
                            System.err.println("OpenAI zeroshot returned empty response");
                            recordEndgameFoul(state, state.currentRound, "Illegal move", "OpenAI");
                        ...
                        System.err.println("OpenAI zeroshot call exception: " + e.getClass().getName() + ": " + e.getMessage());
                        e.printStackTrace();
                        recordEndgameFoul(state, state.currentRound, "Illegal move", "OpenAI");
                        recordEndgameTempError(state, "Illegal move");
                        continue;
                    ...
                    try {
                        OpenAICotClient.OpenAICotResult cotResult = openAICotClient.chatStructured(sessionId, boardDescription);
                        if (cotResult != null && cotResult.isSuccess()) {
                            aiMove = cotResult.getMove();
                            if (aiMove == null || aiMove.isEmpty()) {
                                aiMove = extractMoveFromText(cotResult.getAnswerText());
                            }
                        } else {
                            System.out.println("OpenAI CoT call failed: " +
                        ...
                        System.err.println("OpenAI CoT call exception: " + e.getClass().getName() + ": " + e.getMessage());
                        e.printStackTrace();
                        recordEndgameFoul(state, state.currentRound, "Illegal move", "OpenAI");
                        recordEndgameTempError(state, "Illegal move");
                        continue;
                ...
                if (aiMove != null && !aiMove.isEmpty() && isValidMoveFormat(aiMove)) {
                    if (legalMoves.contains(aiMove)) {
                        ...
                        if (moveApplied) {
                            System.out.println("OpenAI move succeeded (attempt " + attempt + "): " + aiMove);
                            result.put("success", true);
                            result.put("move", aiMove);
                            result.put("attempt", attempt);
                            return result;
                        } else {
                            System.out.println("OpenAI move application failed: " + aiMove);
                            recordEndgameFoul(state, state.currentRound, "Illegal move", "OpenAI");
                            recordEndgameTempError(state, "Illegal move");
                        }
                    } else {
                        System.out.println("OpenAI returned illegal move (attempt " + attempt + "): " + aiMove +
                                " (legal moves: " + legalMoves.size() + ")");
                        recordEndgameFoul(state, state.currentRound, "Illegal move", "OpenAI");
                        recordEndgameTempError(state, "Illegal move");
                    }
                } else {
                    System.out.println("OpenAI returned invalid format (attempt " + attempt + "): " +
                ...
                System.err.println("OpenAI call error (attempt " + attempt + "): " + e.getMessage());
                e.printStackTrace();
                recordEndgameFoul(state, state.currentRound, "Illegal move", "OpenAI");
                recordEndgameTempError(state, "Illegal move");
            }
        ...
        if (!legalMoves.isEmpty()) {
            ...
            System.out.println("OpenAI 1 attempt failed, using random move: " + randomMove);
        ...
        System.out.println("OpenAI cannot provide a legal move");
        result.put("success", false);
        result.put("errorMessage", "OpenAI cannot provide a valid move");
        result.put("attemptsFailed", 1);
        return result;
    }
    ...
    private void recordEndgameTempError(EndgameAIBattleState state, String errorType) {
        ...
        System.out.println("[OpenAI Endgame AI] Recording temp error - Level " + state.level +
    ...
    private void recordEndgameGameResult(EndgameAIBattleState state, String result) {
        ...
        System.out.println("[OpenAI Endgame AI] Recording game result - Level " + state.level +
    ...
    private void recordEndgameFoul(EndgameAIBattleState state, int round, String type, String player) {
        ...
        System.out.println("[OpenAI Endgame AI] Recording foul - Level " + state.level +
    ...
    private void cancelCurrentBattle(String stateKey) {
        ...
        System.out.println("[OpenAI Endgame AI] Cancelling battle: " + stateKey);
    }
