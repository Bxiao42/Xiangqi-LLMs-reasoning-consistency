...
@Service
public class DeepSeekEndgameService {
    ...
    public Map<String, Object> startNewEndgameBattle(int level, String mode, Board initialBoard, Side startingSide) {
        ...
        response.put("message", "Endgame AI battle started - DeepSeek " + (mode.equals("cot") ? "CoT" : "ZeroShot") + "(Red) vs Pikafish(Black)");
        response.put("gameId", state.gameId);
        response.put("level", level);
        response.put("mode", mode);
        response.put("board", serializeBoard(state.board));

        System.out.println("[DeepSeek Endgame AI Battle] Starting new endgame AI battle: Level " + level +
                " - DeepSeek " + (mode.equals("cot") ? "CoT" : "ZeroShot") + "(Red) vs Pikafish(Black)");
        return response;
    }
    ...
    public Map<String, Object> playNextRound(int level, String mode) {
        ...
        System.out.println("[DeepSeek Endgame AI Battle] Level " + level + " Round " + state.currentRound + " starting");

        Map<String, Object> deepseekResult = playDeepSeekMove(state, level, mode);
        if (!(Boolean) deepseekResult.get("success")) {
            state.gameOver = true;
            state.gameResult = "Pikafish wins - DeepSeek cannot continue";
            state.winner = "BLACK";
            recordEndgameFoul(state, state.currentRound, "Illegal move", "DeepSeek");
            recordEndgameGameResult(state, "DeepSeek cannot continue");
            saveEndgameErrorsToCSV(state);
            saveEndgameAccuracyToCSV(state);
            ...
            response.put("foulPlayer", "DeepSeek");
            response.put("foulReason", deepseekResult.get("errorMessage"));
            response.put("boardState", serializeBoard(state.board));
            return response;
        }

        String deepseekMove = (String) deepseekResult.get("move");
        System.out.println("[DeepSeek Endgame AI Battle] DeepSeek" + (mode.equals("cot") ? " CoT" : "") + " move: " + deepseekMove);
        ...
        if (noCrossingPieces(state.board)) {
            ...
            response.put("winner", state.winner);
            response.put("gameResult", state.gameResult);
            response.put("deepseekMove", deepseekMove);
            response.put("round", state.currentRound);
            response.put("boardState", serializeBoard(state.board));
            return response;
        }
        ...
        if (XqEndgameJudge.isCheckmated(state.board, Side.BLACK)) {
            state.gameOver = true;
            state.gameResult = "DeepSeek wins - checkmate";
            state.winner = "RED";
            recordEndgameGameResult(state, "deepseek win");
            saveEndgameErrorsToCSV(state);
            saveEndgameAccuracyToCSV(state);
            response.put("success", true);
            response.put("gameOver", true);
        ...
        List<String> deepseekLegalMoves = getAllLegalMoves(state.board, Side.RED);
        if (deepseekLegalMoves.isEmpty()) {
            state.gameOver = true;
            state.gameResult = "Pikafish wins - DeepSeek has no legal moves";
            state.winner = "BLACK";
        ...
        if (!(Boolean) pikafishResult.get("success")) {
            state.gameOver = true;
            state.gameResult = "DeepSeek wins - Pikafish cannot continue";
            state.winner = "RED";
        ...
        System.out.println("[DeepSeek Endgame AI Battle] Pikafish move: " + pikafishMove);
    ...
    private void saveEndgameAccuracyToCSV(EndgameAIBattleState state) {
        ...
        try {
            ...
            String fullMode = "deepseek-" + state.mode;
    ...
    // Executes DeepSeek's move for the current round.
    private Map<String, Object> playDeepSeekMove(EndgameAIBattleState state, int level, String mode) {
        Map<String, Object> result = new HashMap<>();

        if (XqEndgameJudge.isCheckmated(state.board, Side.RED)) {
            System.out.println("DeepSeek is checkmated, cannot move");
        ...
        if (legalMoves.isEmpty()) {
            System.out.println("DeepSeek is stalemated, cannot move");
            result.put("success", false);
            result.put("errorMessage", "Red is stalemated, no legal moves");
            return result;
        }

        System.out.println("DeepSeek analyzing... legal moves: " + legalMoves.size());
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                if ("zeroshot".equals(mode)) {
                    ...
                    DeepseekCotClient.DeepseekCotResult cotResult = deepseekCotClient.chat(boardDescription);
                    if (cotResult != null && cotResult.isSuccess()) {
                        aiMove = extractMoveFromAnswer(cotResult.getAnswer());
                    } else {
                        System.out.println("DeepSeek CoT call failed (attempt " + attempt + ")");
                        recordEndgameFoul(state, state.currentRound, "Illegal move", "DeepSeek");
                ...
                if (aiMove != null && isValidMoveFormat(aiMove)) {
                    if (legalMoves.contains(aiMove)) {
                        ...
                        if (moveApplied) {
                            System.out.println("DeepSeek move succeeded (attempt " + attempt + "): " + aiMove);
                            result.put("success", true);
                            result.put("move", aiMove);
                            result.put("attempt", attempt);
                            return result;
                        } else {
                            System.out.println("DeepSeek move application failed (attempt " + attempt + "): " + aiMove);
                            recordEndgameFoul(state, state.currentRound, "Illegal move", "DeepSeek");
                            recordEndgameTempError(state, "Illegal move");
                        }
                    } else {
                        System.out.println("DeepSeek returned illegal move (attempt " + attempt + "): " + aiMove +
                                " (legal moves: " + legalMoves.size() + ")");
                        recordEndgameFoul(state, state.currentRound, "Illegal move", "DeepSeek");
                        recordEndgameTempError(state, "Illegal move");
                    }
                } else if (aiMove != null) {
                    System.out.println("DeepSeek returned invalid format (attempt " + attempt + "): " + aiMove);
                    recordEndgameFoul(state, state.currentRound, "Illegal move", "DeepSeek");
                    recordEndgameTempError(state, "Illegal move");
                } else {
                    System.out.println("DeepSeek returned empty move (attempt " + attempt + ")");
                    recordEndgameFoul(state, state.currentRound, "Illegal move", "DeepSeek");
                    recordEndgameTempError(state, "Illegal move");
                }

            } catch (Exception e) {
                System.err.println("DeepSeek call error (attempt " + attempt + "): " + e.getMessage());
                e.printStackTrace();
                recordEndgameFoul(state, state.currentRound, "Illegal move", "DeepSeek");
                recordEndgameTempError(state, "Illegal move");
            }
        ...
        if (!legalMoves.isEmpty()) {
            ...
            System.out.println("DeepSeek 1 attempt failed, using random move: " + randomMove);
        ...
        System.out.println("DeepSeek cannot provide a legal move");
        result.put("success", false);
        result.put("errorMessage", "DeepSeek cannot provide a valid move");
        result.put("attemptsFailed", 1);
        return result;
    }
    ...
    private void recordEndgameTempError(EndgameAIBattleState state, String errorType) {
        ...
        System.out.println("[DeepSeek Endgame AI] Recording temp error - Level " + state.level +
    ...
    private void recordEndgameGameResult(EndgameAIBattleState state, String result) {
        ...
        System.out.println("[DeepSeek Endgame AI] Recording game result - Level " + state.level +
    ...
    private void recordEndgameFoul(EndgameAIBattleState state, int round, String type, String player) {
        ...
        System.out.println("[DeepSeek Endgame AI] Recording foul - Level " + state.level +
    ...
    private void saveEndgameErrorsToCSV(EndgameAIBattleState state) {
        if (state.currentGameErrors == null || state.currentGameErrors.isEmpty()) {
            System.out.println("[DeepSeek Endgame AI] No error records to save");
            return;
        }
        if ("zeroshot".equals(state.mode)) {
            errorCsvRecorder.saveEndgameErrorsToCsv(
                    state.currentGameErrors,
                    ErrorCsvRecorder.RecordType.DEEPSEEK_ZERO_SHOT_ENDGAME
            );
        } else if ("cot".equals(state.mode)) {
            errorCsvRecorder.saveEndgameErrorsToCsv(
                    state.currentGameErrors,
                    ErrorCsvRecorder.RecordType.DEEPSEEK_COT_ENDGAME
            );
        }
    ...
    private void cancelCurrentBattle(String stateKey) {
        ...
        System.out.println("[DeepSeek Endgame AI] Cancelling battle: " + stateKey);
    }
