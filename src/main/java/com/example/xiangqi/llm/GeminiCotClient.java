...
public class GeminiCotClient {
    ...
    public GeminiCotResult chatStructured(String sessionId, String userMessage) {
        return chatStructured(sessionId, userMessage, 0, "UNKNOWN");
    }

    public GeminiCotResult chatStructured(String sessionId, String userMessage, int round, String player) {
        try {
            ...
            for (Map<String, String> msg : history) {
                ...
                if ("system".equals(role)) promptBuilder.append("System: ").append(content).append("\n\n");
            ...
            System.out.println("[Gemini CoT] Sending API request, model: " + cotModel +
                    ", session: " + sessionId + ", round: " + round +
                    ", history turns: " + (history.size() / 2));

            GenerateContentResponse resp = client.models.generateContent(cotModel, prompt, null);
            String fullText = resp.text();
            if (fullText == null || fullText.isBlank()) {
                return GeminiCotResult.error("API returned empty response");
            }

            String reasoning = extractReasoning(fullText);
            String answer = extractAnswer(fullText);
            String move = extractMoveFromText(answer != null ? answer : fullText);

            String shortAssistantReply;
            if (move != null) {
                shortAssistantReply = "Move: " + move;
            } else {
                shortAssistantReply = "Could not extract valid move";
            }
            history.add(Map.of("role", "assistant", "content", shortAssistantReply));

            sessionTurnCount.put(sessionId, sessionTurnCount.getOrDefault(sessionId, 0) + 1);

            System.out.println("[Gemini CoT] Suggested move: " + (move != null ? move : "not found"));
            return GeminiCotResult.success(reasoning, answer, move);

        } catch (Exception e) {
            String errorMsg = "Gemini CoT API call failed: " + e.getMessage();
            System.err.println("[Gemini CoT] " + errorMsg);
            e.printStackTrace();
            return GeminiCotResult.error(errorMsg);
        }
    }
    ...
    public String getXiangqiMove(String sessionId, String boardState, String currentPlayer, int round) {
        ...
        System.out.println("[Gemini CoT Xiangqi] Session: " + sessionId + ", round: " + round);
        GeminiCotResult result = chatStructured(sessionId, boardState, round, currentPlayer);
        if (!result.isSuccess()) return null;
        return result.getMove();
    }

    public String chat(String sessionId, String userMessage) {
        GeminiCotResult result = chatStructured(sessionId, userMessage, 0, "UNKNOWN");
        if (result.isSuccess()) return result.getAnswer() != null ? result.getAnswer() : "No answer generated";
        return "Gemini CoT Error: " + result.getError();
    }
    ...
    public void clearSessionHistory(String sessionId) {
        if (sessionId != null && !sessionId.isBlank()) {
            ...
            System.out.println("[Gemini CoT] Cleared session: " + sessionId);
        }
    }
    ...
    public static class GeminiCotResult {
        ...
        private GeminiCotResult(boolean success, String reasoning, String answer, String move, String error) {
        ...
        public static GeminiCotResult success(String reasoning, String answer, String move) {
            return new GeminiCotResult(true, reasoning, answer, move, null);
        }

        public static GeminiCotResult error(String error) {
            return new GeminiCotResult(false, null, null, null, error);
        }
