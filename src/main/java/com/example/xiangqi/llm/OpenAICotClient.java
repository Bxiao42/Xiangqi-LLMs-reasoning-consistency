...
@Service
public class OpenAICotClient {

    @Value("${openai.cot.api.url:https://api.openai.com/v1/responses}")
    private String apiUrl;

    @Value("${openai.cot.api.key}")
    private String apiKey;

    @Value("${openai.cot.api.model:gpt-5.2}")
    private String model;

    @Value("${openai.cot.reasoning.effort:high}")
    private String reasoningEffort;

    @Value("${openai.cot.max_output_tokens:800}")
    private int maxOutputTokens;

    @Value("${openai.cot.reasoning.enable-summary:false}")
    private boolean enableReasoningSummary;
    ...
    public OpenAICotResult chatStructured(String sessionId, String userMessage) {
        return chatStructured(sessionId, userMessage, 0, "UNKNOWN");
    }

    public OpenAICotResult chatStructured(String sessionId, String userMessage, int round, String player) {
        try {
            ...
            List<Map<String, String>> inputMessages = new ArrayList<>(history);
            ...
            reasoning.put("effort", normalizeEffort(reasoningEffort));
            if (enableReasoningSummary) reasoning.put("summary", "auto");
            body.put("reasoning", reasoning);
            body.put("max_output_tokens", maxOutputTokens);

            System.out.println("[OpenAI CoT] Sending API request, model: " + model +
                    ", session: " + sessionId + ", round: " + round +
            ...
            String answerText = extractOutputTextFromResponses(root);
            String reasoningSummary = enableReasoningSummary ? extractReasoningSummaryFromResponses(root) : "";

            String fullText = "";
            if (reasoningSummary != null && !reasoningSummary.isBlank()) {
                fullText = reasoningSummary + "\n" + (answerText != null ? answerText : "");
            } else if (answerText != null) {
                fullText = answerText;
            }

            String reasoningExtracted = extractReasoning(fullText);
            String answerExtracted = extractAnswer(fullText);
            String move = extractMoveFromText(answerExtracted != null ? answerExtracted : fullText);
            ...
            System.out.println("[OpenAI CoT] Suggested move: " + (move != null ? move : "not found"));
            return OpenAICotResult.success(reasoningExtracted, answerExtracted, move);

        } catch (HttpClientErrorException e) {
            String errorMsg = "HTTP " + e.getStatusCode() + " - " + e.getResponseBodyAsString();
            System.err.println("[OpenAI CoT] HTTP error: " + errorMsg);
            e.printStackTrace();
            return OpenAICotResult.error(errorMsg);
        } catch (ResourceAccessException e) {
            String errorMsg = "OpenAI API request timeout or network error: " + e.getMessage();
            System.err.println("[OpenAI CoT] " + errorMsg);
            e.printStackTrace();
            return OpenAICotResult.error(errorMsg);
        } catch (Exception e) {
            String errorMsg = "Exception: " + e.getMessage();
            System.err.println("[OpenAI CoT] Unknown exception: " + errorMsg);
            e.printStackTrace();
            return OpenAICotResult.error(errorMsg);
        }
    }
    ...
    public String getXiangqiMove(String sessionId, String boardState, String currentPlayer, int round) {
        ...
        System.out.println("[OpenAI CoT Xiangqi] Session: " + sessionId + ", round: " + round);
        OpenAICotResult result = chatStructured(sessionId, boardState, round, currentPlayer);
        if (!result.isSuccess()) return null;
        return result.getMove();
    }

    public String chat(String sessionId, String userMessage) {
        OpenAICotResult r = chatStructured(sessionId, userMessage, 0, "UNKNOWN");
        if (r.isSuccess()) return r.getAnswerText();
        return "OpenAI CoT Error: " + r.getError();
    }
    ...
    public void clearSessionHistory(String sessionId) {
        if (sessionId != null && !sessionId.isBlank()) {
            ...
            System.out.println("[OpenAI CoT] Cleared session: " + sessionId);
        }
    }
    ...
    private String normalizeEffort(String effort) {
        if (effort == null) return "high";
        String e = effort.trim().toLowerCase();
        Set<String> allowed = Set.of("minimal", "low", "medium", "high", "xhigh");
        return allowed.contains(e) ? e : "high";
    }
    ...
    private String extractOutputTextFromResponses(JsonNode root) {
        ...
        String allText = extractAllTextFromJson(root);
        if (!allText.isBlank()) return allText;
        return null;
    }

    private String extractAllTextFromJson(JsonNode node) {
        if (node == null) return "";
        if (node.isTextual()) return node.asText();
        if (node.isObject()) {
            StringBuilder sb = new StringBuilder();
            node.fields().forEachRemaining(entry -> {
                String text = extractAllTextFromJson(entry.getValue());
        ...
        if (node.isArray()) {
            ...
            for (JsonNode child : node) {
                String text = extractAllTextFromJson(child);
                if (!text.isBlank()) sb.append(text).append(" ");
            }
            return sb.toString().trim();
        }
    ...
    private String extractReasoningSummaryFromResponses(JsonNode root) {
        if (root == null) return null;
        JsonNode output = root.path("output");
        ...
        for (JsonNode item : output) {
            ...
            JsonNode summaryArr = item.path("summary");
            if (!summaryArr.isArray()) continue;
            StringBuilder sb = new StringBuilder();
            for (JsonNode s : summaryArr) {
                String sType = s.path("type").asText("");
                if ("summary_text".equals(sType)) {
    ...
    private String extractReasoning(String text) {
        ...
        for (String line : lines) {
            ...
            if (trimmed.startsWith("推理：") || trimmed.startsWith("分析：") ||
                    trimmed.startsWith("思考：") || trimmed.contains("推理") ||
                    trimmed.startsWith("Reasoning:")) {
                inReasoning = true;
            }
            if (inReasoning) {
                reasoning.append(line).append("\n");
                if (trimmed.startsWith("最终答案：") || trimmed.startsWith("最终移动：") ||
                        trimmed.startsWith("Final answer:") || trimmed.startsWith("Move:")) {
                    break;
                }
            }
        }
        if (reasoning.length() > 0) return reasoning.toString().trim();
        int splitIndex = text.length() * 2 / 3;
        return text.substring(0, Math.min(splitIndex, text.length())).trim();
    }
    ...
    private String extractAnswer(String text) {
        ...
        for (int i = lines.length - 1; i >= 0; i--) {
            ...
            if (line.startsWith("最终答案：") || line.startsWith("最终移动：") ||
                    line.startsWith("Final answer:") || line.startsWith("Move:")) {
                return line;
            }
        }
    ...
    public static class OpenAICotResult {
        ...
        private OpenAICotResult(boolean success, String reasoningSummary, String answerText, String move, String error) {
        ...
        public static OpenAICotResult success(String reasoningSummary, String answerText, String move) {
            return new OpenAICotResult(true, reasoningSummary, answerText, move, null);
        }

        public static OpenAICotResult error(String error) {
            return new OpenAICotResult(false, null, null, null, error);
        }
